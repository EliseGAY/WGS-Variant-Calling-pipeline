...
# arg
#-------#
# rate_homALT_max_POS (double) : Max percent of heterozygous site in one position
# rate_homALT_max_Ind (double) : Max percent of heterozygous site in one individus
...
# Usage :
#--------#
# Filter_VCF=Filters_Het_Ind_Pos(vcf_file, rate_homALT_max_POS,rate_homALT_max_Ind)

# Function 
#-------------------------------------------------------------------------------------------------------------------------------------------#
# Get genotype from vfc with the vcfR package
# Count number of "1/1" per column and compute hom alt rate (percent) by divided by the number of line (position)
# Create new vcf with only samples which pass the filter
# On the new VCF : Count number of "1/1" per line and compute hom alt rate (percent) by divided by the number of column (individues)
# Create new vcf with only samples AND position which pass the filter
#-------------------------------------------------------------------------------------------------------------------------------------------#

Filters_hom_Ind_Pos<-function(vfc,rate_homALT_max_POS,rate_homALT_max_Ind){
	# Filters_Het_Ind_Pos(vcf_file, rate_homALT_max_POS,rate_homALT_max_Ind)
	...
	somme_het_ind = colSums(gt=="1/1")
	rate_het_ind = (somme_het_ind/dim(gt)[1])*100
	samples_kept<-names(which(rate_het_ind<rate_homALT_max_Ind))
	...
	somme_het_Pos = rowSums(gt_1=="1/1")
	rate_het_Pos = (somme_het_Pos/dim(gt_1)[2])*100
	Pos_kept<-which(rate_het_Pos<rate_homALT_max_POS)
